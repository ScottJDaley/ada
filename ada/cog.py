...
class Cog(commands.Cog):
    ...
    async def help(self, interaction: discord.Interaction) -> None:
        """ /help """
        query = "help"
        await self.query_and_respond(query, interaction.response)
    ...
    async def query_and_respond(self, query: str, response: discord.InteractionResponse):
        result = await self.ada.do(query)
        message = result.messages(Breadcrumbs.create(query))[0]
        await response.send_message(content=message.content, embed=message.embed, file=message.file, ephemeral=True)
